...
let Productions = {
  ...
  efficiencySettings: Object.assign(
    JSON.parse(
      localStorage.getItem("Productions.efficiencySettings") ||
        `{
			"tilevalues":false,
			"showitems":true,
  ...
  Rating: {
    ...
    getDefaultData: () => ({
      ...
      guild_raids_supplies_production: {
        order: 72,
        perTile: 1,
        active: false,
        group: 3,
    ...
    normalizeData: (data) =>
      Object.assign(
        Productions.Rating.getDefaultData(),
        Productions.Rating.cloneData(data || {}),
      ),
     ...
     */
    updateTypes: () => {
      Productions.Rating.Types = Object.keys(Productions.Rating.Data).sort(
        (a, b) => {
    ...
    savePresets: () => {
      ...
      localStorage.setItem(
        Productions.Rating.PresetStorageKey,
        JSON.stringify(Productions.Rating.Presets),
      );
    },
    ...
    deletePreset: (presetId) => {
      ...
      if (Productions.Rating.Presets.activePresetId === presetId) {
        Productions.Rating.Presets.activePresetId =
          Object.keys(Productions.Rating.Presets.presets)[0] || null;
      }
      return true;
    },
    ...
    ensurePresets: () => {
      ...
      if (!Productions.Rating.Presets) {
        const legacyData = JSON.parse(
          localStorage.getItem(Productions.Rating.LegacyStorageKey) || "{}",
        );
      ...
      if (
        !Productions.Rating.Presets.presets ||
        Object.keys(Productions.Rating.Presets.presets).length === 0
      ) {
        const presetId = "default";
        Productions.Rating.Presets = {
          activePresetId: presetId,
      ...
      if (
        ...
        Productions.Rating.Presets.activePresetId = Object.keys(
          Productions.Rating.Presets.presets,
        )[0];
        Productions.Rating.savePresets();
      }
      ...
      reader.onload = (evt) => {
        try {
          ...
          for (const [id, preset] of Object.entries(presets)) {
            ...
            importedPresets[id] = {
              data: Productions.Rating.normalizeData(preset.data),
            };
          }
          ...
          Productions.Rating.Presets = {
            activePresetId:
              data.activePresetId && importedPresets[data.activePresetId]
                ? data.activePresetId
    ...
    load: (overwrite = null) => {
      ...
      if (localStorage.getItem("ProductionRatingProdPerTiles")) {
        let RatingProdPerTiles = Object.assign(
          {},
          JSON.parse(
            localStorage.getItem("ProductionRatingProdPerTiles") || "{}",
          ),
        );
  ...
  init: () => {
    ...
    MainParser.CityBuildingsData = CityBuildings.createBuildings(
    ...
    if (CityMap.EraOutpost.data) {
      Productions.CombinedCityMapData = Object.assign(
        {},
        Productions.CombinedCityMapData,
        CityMap.EraOutpost.data,
      );
    }
  ...
  ReadData: () => {
    ...
    ((Productions.PopulationSum = 0), (Productions.HappinessSum = 0));

    Productions.BuildingsAll.forEach((building) => {
      if (building.happiness) Productions.HappinessSum += building.happiness;
      if (building.population && building.population > 0)
        Productions.PopulationSum += building.population;
    });

    let ProdBonus = 0;
    if (Productions.HappinessSum < Productions.PopulationSum) ProdBonus = -0.5;
        ...
        Productions.getBoost(building, boost, function (result) {
          if (result !== undefined) {
            if (Productions.BuildingsProducts[boost]) {
              if (
                Productions.BuildingsProducts[boost].find(
                  (x) => x.id === building.id,
                ) === undefined
              )
            ...
            if (boost.includes("guild_raids")) {
              if (
                Productions.BuildingsProducts.guild_raids.find(
                  (x) => x.id === building.id,
                ) === undefined
              )
      ...
      if (building.production) {
        building.production.forEach((production) => {
          if (production.type === "guildResources") {
            if (
              Productions.BuildingsProducts.clan_goods.find(
                (x) => x.id === building.id,
              ) === undefined
            )
          ...
          if (production.type === "unit") {
            if (
              Productions.BuildingsProducts.units.find(
                (x) => x.id === building.id,
              ) === undefined
            )
          ...
          if (production.type === "random") {
            production.resources.forEach((resource) => {
              ...
              if (
                resource.type === "forgepoint_package" ||
                resource.subType === "forgepoint_package"
              ) {
                // e.g. grilling grove
                if (
                  Productions.BuildingsProducts.strategy_points.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
                  Productions.BuildingsProducts["strategy_points"].push(
                    saveBuilding,
                  );
              }
              if (
                resource.type === "consumable" ||
                resource.type.includes("chest")
              ) {
                if (
                  Productions.BuildingsProducts.items.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
              ...
              if (
                ...
                resource.subType === "strategy_points"
              ) {
                if (
                  Productions.BuildingsProducts.strategy_points.find(
                    (x) => x.id === building.id,
              ...
              if (
                resource.type === "resources" &&
                resource.subType === "money"
              ) {
                if (
                  Productions.BuildingsProducts.money.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
              ...
              if (
                resource.type === "resources" &&
                resource.subType === "supplies"
              ) {
                if (
                  Productions.BuildingsProducts.supplies.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
              ...
              if (
                resource.type === "resources" &&
                resource.subType === "medals"
              ) {
                if (
                  Productions.BuildingsProducts.medals.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
              ...
              if (
                resource.type.includes("good") &&
                !resource.type.includes("guild")
              ) {
                if (
                  Productions.BuildingsProducts.goods.find(
                    (x) => x.id === building.id,
                  ) === undefined
                )
          ...
          if (production.type === "special_goods") {
            if (
              Productions.BuildingsProducts.special_goods.find(
                (x) => x.id === building.id,
              ) === undefined
            ) {
          ...
          if (production.type === "resources") {
            ...
            if (
              types.find(
                (x) =>
                  x.includes("random_good_of_") || x.includes("all_goods_of_"),
              )
            ) {
              if (
                Productions.BuildingsProducts.goods.find(
      ...
      if (building.state.production) {
        building.state.production.forEach((production) => {
          ...
          if (production.type === "resources") {
            ...
            if (production.resources.premium) {
              if (
                Productions.BuildingsProducts.premium.find(
                  (x) => x.id === building.id,
                ) === undefined
              )
                Productions.BuildingsProducts.premium.push(saveBuilding);
  ...
  setChainsAndSets(buildings) {
    if (buildings === undefined)
      buildings = Object.values(MainParser.CityBuildingsData);
    let idsToRemove = [];

    for (const building of buildings) {
      if (building?.setBuilding !== undefined) {
        let adjacentIds =
          CityBuildings.findAdjacentSetBuildingByCoords(building);
        ...
        building?.chainBuilding !== undefined &&
        building?.chainBuilding?.type === "start"
      ) {
    ...
    if (idsToRemove.length > 0) {
      Productions.BuildingsAll = Productions.BuildingsAll.filter(
        (b) => !idsToRemove.includes(b.id),
      );
    }
  },
  ...
  buildQITable(type) {
    ...
    buildingIds.forEach((b) => {
      ...
      if (building.player_id === ExtPlayerID) {
        ...
        rowA.push("<td>");
        rowA.push(
          building.state.isPolivated !== undefined
            ? building.state.isPolivated
        ...
        if (building.setBuilding !== undefined)
          rowA.push(
            '<img src="' +
              srcLinks.get(
                "/shared/icons/" + building.setBuilding.name + ".png",
                true,
              ) +
              '" class="chain-set-ico">',
          );
        if (building.chainBuilding !== undefined)
          rowA.push(
            '<img src="' +
              srcLinks.get(
                "/shared/icons/" + building.chainBuilding.name + ".png",
                true,
              ) +
              '" class="chain-set-ico">',
          );
        ...
        rowA.push(
          '<td data-text="' +
            helper.str.cleanup(building.name) +
            '" class="' +
            (MainParser.Allies.buildingList?.[building.id] ? "ally" : "") +
            '">' +
            building.name +
            "</td>",
        );
        ...
        if (building.boosts !== undefined) {
          ...
          building.boosts.forEach((boost) => {
            if (boost.type.find((x) => x.includes("guild_raids_"))) {
              ...
              else boosts[boost.type[0]] += parseInt(boost.value);

              if (boostCounter[boost.type[0]] === undefined)
                boostCounter[boost.type[0]] = parseInt(boost.value);
              else boostCounter[boost.type[0]] += parseInt(boost.value);
            }
          });
              ...
              if (boosts[type] !== undefined)
                rowA.push(
                  '<td data-number="' +
                    boosts[type] +
                    '" class="text-center">' +
                    HTML.Format(boosts[type]) +
                    "</td>",
                );
              else
                rowA.push(
                  '<td data-number="' + 0 + '" class="text-center">-</td>',
                );
            }
          }
        }
        ...
        rowA.push(
          '<td data-number="' +
            Technologies.Eras[building.eraName] +
            '">' +
            i18n("Eras." + Technologies.Eras[building.eraName] + ".short") +
            "</td>",
        );
        rowA.push('<td class="text-right">');
        rowA.push(
          '<span class="show-entity" data-id="' +
            building.id +
            '"><img class="game-cursor" alt="" src="' +
            extUrl +
            'css/images/hud/open-eye.png"></span>',
        );
        rowA.push("</td>");
        rowA.push("</tr>");
      }
    ...
    if (rowA.length > 0) {
      table.push(
        '<table id="' +
          type +
          '-list" class="foe-table sortable-table TSinactive ' +
          type +
          '-list active exportable">',
      );
      table.push('<thead class="sticky">');
      table.push("<tr>");
      table.push(
        '<th colspan="12"><input type="text" placeholder="' +
          i18n("Boxes.Productions.FilterTable") +
          '" class="filterCurrentList">' +
          "<span class=\"btn\" onclick=\"Productions.createBuildingBoostList(['guild_raids_action_points_collection','guild_raids_coins_production','guild_raids_coins_start','guild_raids_supplies_production','guild_raids_supplies_start','guild_raids_goods_start','guild_raids_units_start'])\">" +
          i18n("Boxes.BoostList.open") +
          "</span></th>",
      );
      table.push("</tr>");
      ...
      table.push(
        '<th class="ascending" data-type="prodlist' +
          type +
          '" data-export="' +
      ...
      table.push(
        '<th class="boost qiactions is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_action_points_collection || 0) +
          "</th>",
      );
      table.push(
        '<th class="boost qicapacity is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_action_points_capacity || 0) +
          "</th>",
      );
      table.push(
        '<th class="boost qicoins is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_coins_production || 0) +
          "%</th>",
      );
      table.push(
        '<th class="boost qicoins_start is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_coins_start || 0) +
          "</th>",
      );
      table.push(
        '<th class="boost qisupplies is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_supplies_production || 0) +
          "%</th>",
      );
      table.push(
        '<th class="boost qisupplies_start is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_supplies_start || 0) +
          "</th>",
      );
      table.push(
        '<th class="boost qigoods_start is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_goods_start || 0) +
          "</th>",
      );
      table.push(
        '<th class="boost qiunits_start is-number text-center" data-type="prodlist' +
          type +
          '"><span></span>' +
          (boostCounter.guild_raids_units_start || 0) +
          "</th>",
      );
      table.push(
        '<th data-type="prodlist' +
          type +
          '" class="is-number">' +
          i18n("Boxes.Productions.Headings.era") +
          "</th>",
      );
      ...
      table.push(
        '<div class="empty-list">' +
          i18n("Boxes.Productions.EmptyList") +
          "</div>",
      );
    }
  ...
  buildTableByType(type) {
    ...
    if (type !== "goods" && type !== "clan_goods" && type !== "guild_raids") {
      for (const b of buildingIds) {
        ...
        if (
          (type === "items" &&
            Productions.showBuildingItems(true, building)[0] === "") ||
          building.chainBuilding?.type === "linked"
        )
          continue;
        ...
        if (!type.includes("att") && !type.includes("def")) {
          if (type !== "items") {
            let currentProductionByCategory =
              Productions.getBuildingProductionByCategory(true, building, type);
            let generalProductionByCategory =
            ...
            hasRandomProductions =
              generalProductionByCategory.hasRandomProductions;
            ...
            if (type === "money" && building.isBoostable) {
              amount =
                Math.round(
                  amount +
                    amount *
                      ((Boosts.Sums.coin_production +
                        Productions.HappinessBoost * 100) /
                        100),
                ) * (doubled ? 2 : 1);
              ...
              amount = Math.round(
                amount + amount * (Boosts.Sums.forge_points_production / 100),
              );
              currentAmount = Math.round(
                currentAmount +
                  currentAmount * (Boosts.Sums.forge_points_production / 100),
              );
            }
            ...
            let parsedCurrentAmount =
              currentAmount >= 10000
                ? HTML.FormatNumberShort(currentAmount)
                : HTML.Format(currentAmount);
            let parsedAmount =
              currentAmount >= 10000
                ? HTML.FormatNumberShort(amount)
                : HTML.Format(amount);

            if (
              generalProductionByCategory.units.length > 0 ||
              currentProductionByCategory.units.length > 0
            ) {
              if (currentProductionByCategory.units.length > 0)
                rowA.push(
                  currentProductionByCategory.units
                    .map(
                      (x) =>
                        `${x.amount}<span class="unit_skill ${x.type} ${x.era > CurrentEraID ? "next_era" : ""}" title="${i18n("Boxes.Units." + x.type)}"></span> `,
                    )
                    .join(" "),
                );
              else rowA.push(" - ");
              ...
              if (generalProductionByCategory.units.length > 0)
                rowA.push(
                  generalProductionByCategory.units
                    .map(
                      (x) =>
                        `${x.amount ? x.amount : ""}${x.amount && x.random ? "+" : ""}${x.random ? "Ø" + x.random : ""}<span class="unit_skill ${x.type} ${x.era > CurrentEraID ? "next_era" : ""}" title="${i18n("Boxes.Units." + x.type)}"></span> `,
                    )
                    .join(" "),
                );
              else rowA.push(" - ");
              ...
              if (currentAmount < amount && building.type !== "production")
                ...
                if (unitType !== null) {
                  rowA.push(
                    '<span class="unit_skill ' +
                      unitType.replace(/next./, "") +
                      '" title="' +
                      i18n("Boxes.Units." + unitType.replace(/next./, "")) +
                      '"></span> ',
                  );
                }
            ...
            for (let i of items[2]) {
              let n =
                (i.fragment ? "Fragment" : "") + i.name.replace(/\s/g, "");
              if (Sum[n]) {
            ...
            rowA.push(
              '<td data-number="1" exportvalue="' +
                jQuery(itemsText).text() +
                '"><span>' +
                itemsText +
                "</span></td>",
            );
          ...
          if (building.boosts !== undefined) {
            ...
            rowA.push(
              '<td data-number="' +
                ...
                boosts.all +
                '" class="text-center">' +
                (boosts.all !== 0 ? HTML.Format(boosts.all) : "-") +
                "</td>",
            );
            rowA.push(
              '<td data-number="' +
                ...
                boosts.battleground +
                '" class="text-center">' +
                (boosts.battleground !== 0
                  ? HTML.Format(boosts.battleground)
                  : "-") +
                "</td>",
            );
            rowA.push(
              '<td data-number="' +
                boosts.guild_expedition +
                '" exportvalue="' +
                boosts.guild_expedition +
                '" class="text-center">' +
                (boosts.guild_expedition !== 0
                  ? HTML.Format(boosts.guild_expedition)
                  : "-") +
                "</td>",
            );
            rowA.push(
              '<td data-number="' +
                ...
                boosts.guild_raids +
                '" class="text-center">' +
                (boosts.guild_raids !== 0
                  ? HTML.Format(boosts.guild_raids)
                  : "-") +
                "</td>",
            );
          }
        ...
        let updateGroup = groupedBuildings.find(
          (x) => x.building.name === building.name,
        );
        ...
        rowA.push(
          "<td " +
            (type.includes("att") || type.includes("def")
              ? 'colspan="3"'
              : "") +
            ' data-number="' +
            Technologies.Eras[building.eraName] +
        ...
        if (!type.includes("att") && !type.includes("def")) {
          let time = "-";
          let showRelativeProductionTime = JSON.parse(
            localStorage.getItem("productionsShowRelativeTime") || "false",
          );
          let showAMPMTime = JSON.parse(
            localStorage.getItem("productionsShowAMPMTime") || "false",
          );
          if (building.state.times?.at) {
            if (showRelativeProductionTime)
              ...
              if (building.state.times?.at <= inADay)
                ...
                time = moment
                  .unix(building.state.times?.at)
                  .format("dddd, HH:mm");
            }
          }
          let done =
            building.state.name === "collectable"
              ? i18n("Boxes.Productions.Done")
              : "";
          rowA.push(
            '<td style="white-space:nowrap" data-date="' +
              (building.state.times?.at || 9999999999) +
              '">' +
              (done == "" ? time : '<b class="text-success">' + done + "</b>") +
              "</td>",
          );
        }
        rowA.push('<td class="text-right">');
    ...
    if (rowA.length > 0) {
      ...
      table.push("<tr>");
      table.push(
        '<th colspan="3"><span class="btn change-view game-cursor" data-type="' +
          type +
          '">' +
          i18n("Boxes.Productions.ModeGroups") +
          '</span> <input type="text" placeholder="' +
          i18n("Boxes.Productions.FilterTable") +
          '" class="filterCurrentList"></th>',
      );
      if (!type.includes("att") && !type.includes("def") && type != "items") {
        table.push('<th colspan="3" class="textright">');
        table.push(
          (typeCurrentSum >= 10000
            ? HTML.FormatNumberShort(typeCurrentSum)
            : HTML.Format(typeCurrentSum)) +
            "/" +
            (typeSum >= 10000
              ? HTML.FormatNumberShort(typeSum)
              : HTML.Format(typeSum)),
        );
        if (type === "strategy_points") {
          table.push(
            ' <button class="typeBoost btn btn-slim"><a href="#forge_points_production" class="game-cursor">' +
              i18n("General.Boost") +
              ": " +
              Boosts.Sums.forge_points_production +
              "%</a></button>",
          );
          ...
          table.push(
            ' <button class="typeBoost btn btn-slim"><a href="#coin_production" class="game-cursor">' +
              i18n("General.Boost") +
              ": " +
              Boosts.Sums.coin_production +
              "%</a></button>",
          ...
          table.push(
            ' <button class="typeBoost btn btn-slim"><a href="#supply_production" class="game-cursor">' +
              i18n("General.Boost") +
              ": " +
              Boosts.Sums.supply_production +
              "%</a></button>",
        ...
        table.push(
          '<th colspan="8" class="textright">' +
            (type == "items"
              ? '<span class="btn" onclick="Productions.showItemSources(event)" style="float:right;">' +
                i18n("Boxes.ItemSources.Title") +
                "</span>"
              : "") +
            "</th>",
        );
      ...
      if (!type.includes("att") && !type.includes("def"))
        ...
        table.push(
          '<th class="boost ' +
            type +
            ' is-number text-center" data-type="prodlist' +
            type +
            '" data-export="' +
            boostCounter[type].all +
            '"><span></span>' +
            boostCounter[type].all +
            "</th>",
        );
        table.push(
          '<th class="boost battleground is-number text-center" data-type="prodlist' +
            type +
            '" data-export="' +
            boostCounter[type].battleground +
            '"><span></span>' +
            boostCounter[type].battleground +
            "</th>",
        );
        table.push(
          '<th class="boost guild_expedition is-number text-center" data-type="prodlist' +
            type +
            '" data-export="' +
            boostCounter[type].guild_expedition +
            '"><span></span>' +
            boostCounter[type].guild_expedition +
            "</th>",
        );
        table.push(
          '<th class="boost guild_raids is-number text-center" data-type="prodlist' +
            type +
            '" data-export="' +
            boostCounter[type].guild_raids +
            '"><span></span>' +
            boostCounter[type].guild_raids +
            "</th>",
        );
      ...
      if (!type.includes("att") && !type.includes("def")) {
        table.push(
          '<th class="is-date" data-type="prodlist' +
            type +
            '">' +
            i18n("Boxes.Productions.Headings.earning") +
            "</th>",
        );
      }
      table.push(
        '<th data-type="prodlist' +
          type +
          '" class="no-sort" ' +
          (type.includes("att") || type.includes("def") ? 'colspan="3"' : "") +
          "> </th>",
      );
      table.push("</tr>");
      table.push("</thead>");
      ...
      tableGr = Productions.buildGroupedTable(
        type,
        groupedBuildings,
        boostCounter,
      );
    ...
    if (type === "guild_raids") content = Productions.buildQITable(type);
  ...
  getRelevantGoodsByEra: (buildingIds, guildGoods = false) => {
    ...
    for (const b of buildingIds) {
      ...
      let allGoods = {};
      if (guildGoods)
        allGoods = CityBuildings.getBuildingGuildGoodsByEra(
          false,
          building,
          true,
        );
      ...
      for (const [era, value] of Object.entries(allGoods.eras)) {
        if (eras.find((x) => x == era) == undefined) eras.push(parseInt(era));
      }
    }
  ...
  buildGoodsTable: (buildingIds, type = "goods") => {
    ...
    for (const b of buildingIds) {
      ...
      let currentGoods = Productions.getBuildingProductionByCategory(
        true,
        building,
        type,
      );
      let allGoods = Productions.getBuildingProductionByCategory(
        false,
        building,
        type,
      );
      ...
      for (const era of eras) {
        ...
        if (allGoods !== undefined) {
          erasCurrent[era] += currentGoodAmount =
            currentGoods?.eras?.[era] || 0;
        ...
        if (currentGoodAmount !== goodAmount) {
          ...
          rowA.push(
            HTML.Format(currentGoodAmount) +
              "/" +
              isAverage +
              HTML.Format(goodAmount),
          );
        } else rowA.push(HTML.Format(goodAmount));
        rowA.push("</td>");
      }
    ...
    table.push(
      '<table id="' +
        type +
        '-list" class="foe-table sortable-table exportable TSinactive ' +
        type +
        '-list active">',
    );
    table.push('<thead class="sticky">');
    table.push("<tr>");
    table.push(
      '<th colspan="5"><span class="btn change-view game-cursor" data-type="' +
        type +
        '">' +
        i18n("Boxes.Productions.ModeGroups") +
        '</span> <input type="text" placeholder="' +
    ...
    table.push(
      `<th colspan=${eras.length + 1} class="textright">${HTML.Format(Object.values(erasCurrent).reduce((a, b) => a + b))}/${HTML.Format(Object.values(erasTotal).reduce((a, b) => a + b))} 	<button class="typeBoost btn btn-slim"><a href="#special_goods" class="game-cursor">★</a></button> </th>`,
    );
    table.push("</tr>");
    ...
    for (const era of eras) {
      table.push(
        '<th data-type="prodlist' +
          type +
          '" class="is-number text-center"><span data-original-title="' +
          i18n("Eras." + (parseInt(era) + 1)) +
          '">' +
          i18n("Eras." + (parseInt(era) + 1) + ".short") +
          "<br><small>" +
          HTML.Format(erasCurrent[era]) +
          "/" +
          HTML.Format(erasTotal[era]) +
          "</small></span></th>",
      );
    }
    table.push(
    ...
    // grouped view
    table.push(
      '<table class="foe-table sortable-table TSinactive ' + type + '-group">',
    );
    table.push('<thead class="sticky">');
    table.push("<tr>");
    table.push(
      '<th><span class="btn change-view game-cursor" data-type="' +
        type +
        '">' +
    ...
    table.push(
      `<th colspan=${2 + eras.length} class="textright">${HTML.Format(Object.values(erasCurrent).reduce((a, b) => a + b))}/${HTML.Format(Object.values(erasTotal).reduce((a, b) => a + b))}</th>`,
    );
    table.push("</tr>");
    table.push('<tr class="sorter-header">');
    table.push(
      '<th data-type="prodgroup' +
        type +
        '" class="is-number">' +
        i18n("Boxes.Productions.Headings.number") +
        "</th>",
    ...
    table.push(
      '<th data-type="prodgroup' +
        type +
        '" class="is-number">' +
        i18n("Boxes.Productions.Headings.size") +
        "</th>",
    );
    table.push("</tr>");
    table.push("</thead>");
    ...
    for (const building of groupedBuildings) {
      ...
      rowB.push(
        '<td data-text="' +
          building.building.name.replace(/[. -]/g, "") +
          '"  class="' +
          (MainParser.Allies.buildingList?.[building.building.id]
            ? "ally"
            : "") +
          '">' +
      ...
      for (const era of eras) {
        rowB.push(
          '<td data-number="' + building[era] + '" class="text-center">',
        );
      ...
      rowB.push(
        '<td data-number="' +
          building.building.size.length * building.building.size.width +
          '">' +
          building.building.size.length +
          "x" +
          building.building.size.width +
          "</td>",
      );
      rowB.push("</tr>");
    }
  ...
  buildGuildGoodsTable: (buildingIds, type = "clan_goods") => {
    ...
    for (const b of buildingIds) {
      ...
      let currentGoods = CityBuildings.getBuildingGuildGoodsByEra(
        true,
        building,
        true,
      );
      let allGoods = CityBuildings.getBuildingGuildGoodsByEra(
        false,
        building,
        true,
      );
    ...
    table.push(
      '<th colspan="' +
        (6 + eras.length) +
        '"><span class="btn change-view game-cursor" data-type="' +
        type +
        '">' +
        i18n("Boxes.Productions.ModeGroups") +
        '</span> <input type="text" placeholder="' +
    ...
    table.push(
      `<span style="float:right;">${HTML.Format(Object.values(erasCurrent).reduce((a, b) => a + b))}/${HTML.Format(Object.values(erasTotal).reduce((a, b) => a + b))}</span></th>`,
    );
    table.push("</tr>");
    table.push('<tr class="sorter-header exportheader">');
    table.push('<th class="no-sort" data-type="prodlist' + type + '"> </th>');
  ...
  buildGroupedTable: (type, groupedBuildings, boostCounter) => {
    ...
    tableGr.push("<tr>");
    tableGr.push(
      '<th colspan="7"><span class="btn change-view game-cursor" data-type="' +
        type +
        '">' +
        (type == "items" || type == "units"
          ? i18n("Boxes.Productions.ModeSum")
          : i18n("Boxes.Productions.ModeSingle")) +
        "</span></th>",
    );
    tableGr.push("</tr>");
    ...
    if (type.includes("att") || type.includes("def")) {
      tableGr.push(
        '<th class="boost ' +
          type +
          ' is-number text-center" data-type="prodgroup' +
          type +
          '"><span></span>' +
          boostCounter[type].all +
          "</th>",
      ...
      tableGr.push(
        '<th class="boost battleground is-number text-center" data-type="prodgroup' +
          type +
          '"><span></span>' +
          boostCounter[type].battleground +
          "</th>",
      ...
      tableGr.push(
        '<th class="boost guild_expedition is-number text-center" data-type="prodgroup' +
          type +
          '"><span></span>' +
          boostCounter[type].guild_expedition +
          "</th>",
      ...
      tableGr.push(
        '<th class="boost guild_raids is-number text-center" data-type="prodgroup' +
          type +
          '"><span></span>' +
          boostCounter[type].guild_raids +
          "</th>",
      ...
      tableGr.push(
        '<th colspan="4" data-type="prodgroup' +
          type +
          '" class="is-number">' +
          i18n("Boxes.Productions.Headings.number") +
          "</th>",
    ...
    groupedBuildings.forEach((building) => {
      ...
      if (type.includes("att") || type.includes("def")) {
        rowB.push(
          '<td data-number="' +
            building.boosts.all * building.amount +
            '" class="text-center">' +
            (building.boosts.all !== 0
              ? HTML.Format(building.boosts.all * building.amount)
              : "") +
            "</td>",
        );
        rowB.push(
          '<td data-number="' +
            building.boosts.battleground * building.amount +
            '" class="text-center">' +
            (building.boosts.battleground !== 0
              ? HTML.Format(building.boosts.battleground * building.amount)
              : "") +
            "</td>",
        );
        rowB.push(
          '<td data-number="' +
            building.boosts.guild_expedition * building.amount +
            '" class="text-center">' +
            (building.boosts.guild_expedition !== 0
              ? HTML.Format(building.boosts.guild_expedition * building.amount)
              : "") +
            "</td>",
        );
        rowB.push(
          '<td data-number="' +
            building.boosts.guild_raids * building.amount +
            '" class="text-center">' +
            (building.boosts.guild_raids !== 0
              ? HTML.Format(building.boosts.guild_raids * building.amount)
              : "") +
            "</td>",
        );
      } else if (type === "items") {
        rowB.push(
          '<td colspan="4">' +
            Productions.showBuildingItems(false, building.building)[0] +
            "</td>",
        );
      } else {
        if (building.currentValues === building.values)
          rowB.push(
            '<td colspan="4" data-number="' +
              building.currentValues +
              '">' +
              HTML.Format(building.currentValues) +
              "</td>",
          );
        else {
          rowB.push(
            '<td colspan="4" data-number="' +
              ...
              HTML.Format(building.currentValues) +
              "/" +
              (groupedBuildings.hasRandomProductions ? "Ø" : "") +
              HTML.Format(building.values) +
              "</td>",
          );
        }
      }
  ...
  buildSumTable: (type, Sum) => {
    ...
    if (type == "items") {
      ...
      //units
      elements = Object.values(Sum).sort(
        (b, a) =>
          (a.theory?.type == "rogue" ? 100 : 0) +
          (a.theory?.era || 0) -
          ((b.theory?.type == "rogue" ? 100 : 0) + (b.theory?.era || 0)),
      );
    }
    ...
    table.push("<tr>");
    table.push(
      '<th colspan="8"><span class="btn change-view game-cursor">' +
        i18n("Boxes.Productions.ModeSingle") +
        "</span></th>",
    );
    table.push("</tr>");
    ...
    table.push(
      '<th colspan="' +
        ...
        '" >' +
        (type == "items"
          ? i18n("Boxes.Productions.Headings.item")
          : i18n("Boxes.Units.Unit")) +
        "</th>",
    );
    table.push("</tr>");
    table.push("</thead>");
    ...
    for (e of elements) {
      if (type == "items") {
        let amount =
          (e.amount ? parseFloat(Math.round(e.amount * 100) / 100) : "") +
          (e.random && e.amount ? " + " : "") +
          (e.random ? "Ø " + parseFloat(Math.round(e.random * 100) / 100) : "");
        table.push(
          `<tr><td class="text-right">${amount}</td><td>${e.fragment ? "🧩 " : ""}</td><td colspan="5">${e.name}</td></tr>`,
        );
      } else {
        //units
        let currentamount = e.current?.amount
          ? parseFloat(Math.round(e.current.amount * 100) / 100)
          : e.theory?.type !== "random"
            ? "0"
            : "";

        let theoryamount =
          (e.theory?.amount
            ? parseFloat(Math.round(e.theory.amount * 100) / 100)
            : "") +
          (e.theory?.random && e.theory?.amount ? " + " : "") +
          (e.theory?.random
            ? "Ø " + parseFloat(Math.round(e.theory.random * 100) / 100)
            : "");
        theoryamount =
          (currentamount != "" && theoryamount !== "" ? "/ " : "") +
          theoryamount;
        table.push(
          `<tr><td colspan="2">${currentamount} ${theoryamount}</td><td colspan="6"><span class="unit_skill ${(e.theory?.type || e.current.type).replace(/next./, "")}" title="${i18n("Boxes.Units." + (e.theory?.type || e.current.type).replace(/next./, ""))}"></span> <span>${e.theory?.era === 0 || e.current?.era === 0 ? "" : i18n("Eras." + (e.theory?.era || e.current?.era))}</span></td></tr>`,
        );
      }
    }
    table.push("</tbody>");
  ...
  getBuildingProductionByCategory(current = false, building, category) {
    ...
    if (productions) {
      for (let production of productions) {
        ...
        if (production.type === "random") {
          production.resources.forEach((resource) => {
            if (resource.type + "s" === category) {
              ...
              Uera =
                Uera +
                (resource.name.includes("next") &&
                Uera < Technologies.getMaxEra()
                  ? 1
                  : 0);
              let Utype = resource.name;
              prod.units.push({
                type: Utype.replace(/next./, ""),
                amount: 0,
                random: resource.amount * resource.dropChance,
                era: Utype == "rogue" ? 0 : Uera,
              });
            }
            if (
              (resource.type === "guild_goods" && category === "clan_goods") ||
              (resource.subType === "strategy_points" &&
                category === "strategy_points") ||
        ...
        if (
          ...
          let combinedValues =
            Object.entries(production.resources).length === 1
              ? Math.round(
                  (prod.amount * Boosts.Sums.special_goods_production) / 100,
                )
              : 0;
          for (let [type, value] of Object.entries(production.resources)) {
            let boostedValue = Math.round(
              (value * Boosts.Sums.special_goods_production) / 100,
            );
            combinedValues += value + boostedValue;
          }
          prod.amount = combinedValues;
        }
        if (production.type + "s" === category) {
          // units
          ...
          Uera =
            Uera +
            (Utype.includes("next") && Uera < Technologies.getMaxEra() ? 1 : 0);
          prod.amount += UAmount;
          if (!current && building.type === "greatbuilding") {
            let m = Object.values(MainParser.CityMapData).filter(
              (x) => x.type == "military",
            );
            let RAmount = UAmount / m.length;
            m.forEach((x) => {
              let Rtype =
                MainParser.CityEntities[x.cityentity_id].available_products[0]
                  .unit_class;
              if (
                MainParser.CityEntities[x.cityentity_id].available_products[0]
                  .unit_type_id == "rogue"
              )
                Rtype = "rogue"; //Banners + Drummers???
              let Rera =
                Technologies.Eras[
                  MainParser.CityEntities[x.cityentity_id].requirements.min_era
                ];
              prod.units.push({
                type: Rtype.replace(/next./, ""),
                amount: 0,
                random: RAmount,
                era: Rtype == "rogue" ? 0 : Rera,
              });
            });
          } else {
            prod.units.push({
              ...
              random: 0,
              era:
                building.type === "greatbuilding" || Utype == "rogue"
                  ? 0
                  : Uera,
            });
          }
        ...
        if (category === "clan_goods" && production.type === "guildResources") {
          if (production.resources?.all_goods_of_age)
            ...
            if (production.resources !== undefined) {
              let good = GoodsList.find(
                (x) => x.id === Object.keys(production.resources)[0],
              );
              if (good !== undefined)
    ...
    if (
      category === "forge_points_production" ||
      category === "coin_production" ||
      category === "supply_production"
    ) {
      prod.amount = building.boosts.filter(
        (x) => x.type[0] === category,
      )[0].value; // not really rock solid like this
    }
    return prod;
  },
  ...
  showBuildingItems(current = false, building) {
    ...
    if (
      current &&
      (building.state?.isPolivated === true ||
        building.state?.isPolivated === undefined) &&
      Array.isArray(building.state?.production)
    ) {
      for (const production of building.state?.production) {
        ...
        allItems +=
          "<span>" +
          production.resources.amount +
          "x " +
          (frag ? "🧩 " : "") +
          production.resources.name +
          "</span><br>";
        itemArray.push({
          fragment: frag,
          name: production.resources.name,
          amount: production.resources.amount,
          random: 0,
        });
      }
    }
        ...
        for (const production of building.production) {
          if (production.type === "random") {
            for (const resource of production.resources) {
              ...
              let amount = parseFloat(
                Math.round(resource.amount * resource.dropChance * 100) / 100,
              );
              if (resource.type === "unit") {
                allUnits +=
                  "Ø " +
                  ...
                  "x " +
                  (frag ? "🧩 " : "") +
                  `<img src='${srcLinks.get("/shared/icons/" + resource.name.replace(/next./, "").replace("random", "random_production") + ".png", true)}'>` +
                  "<br>";
              } else {
                allItems +=
                  "<span>Ø " +
          ...
          if (production.type === "unit") {
            for (let u of Object.keys(production.resources)) {
              allUnits +=
                production.resources[u] +
                "x " +
                `<img src='${srcLinks.get("/shared/icons/" + u.replace(/next./, "").replace("random", "random_production") + ".png", true)}'>` +
                "<br>";
            }
          }
          if (production.resources?.type === "consumable") {
            ...
            allItems +=
              ...
              "x " +
              (frag ? "🧩 " : "") +
              production.resources.name.replace(/^\d+/, "") +
              "</span><br>";
            itemArray.push({
              fragment: frag,
              name: production.resources.name,
  ...
  readType: (d) => {
    ...
    if (Productions.Boosts["money"] === undefined)
      Productions.Boosts["money"] =
        (Boosts.Sums["coin_production"] + 100) / 100;
    if (Productions.Boosts["supplies"] === undefined)
      Productions.Boosts["supplies"] =
        (Boosts.Sums["supply_production"] + 100) / 100;
    if (Productions.Boosts["fp"] === undefined)
      Productions.Boosts["fp"] =
        (Boosts.Sums["forge_points_production"] + 100) / 100;
  },
  ...
  GetTabs: () => {
    return (
      '<ul class="horizontal dark-bg clickable">' +
      Productions.Tabs.join("") +
      "</ul>"
    );
  },
  ...
  SetTabContent: (id, content) => {
    ...
    let style =
      Productions.TabsContent.length > 0 ? ' style="display:none"' : "";

    Productions.TabsContent.push(
      '<div class="content" id="' + id + '"' + style + ">" + content + "</div>",
    );
  },
  ...
  SwitchFunction: () => {
    $("#Productions").on("click", ".change-view", function () {
      ...
      if (hiddenTable.length == 0)
        hiddenTable = activeTable.siblings("table").first();
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < IDArray.length; i++) {
        let target = document.querySelector(
          '.entity[data-id="' + IDArray[i] + '"]',
        );
        if (target) {
          let targetStyle = window.getComputedStyle(
            document.querySelector('.entity[data-id="' + IDArray[i] + '"]'),
          );
          let tLeft =
            parseInt(targetStyle.getPropertyValue("left").replace("px", "")) -
            100;
          let tTop =
            parseInt(targetStyle.getPropertyValue("top").replace("px", "")) -
            100;
          // todo: andere perspektive beachten, andere werte benutzen?

          if (i === 0)
            $("#map-container").scrollTo({ left: tLeft, top: tTop }, 800, {
              easing: "swing",
            });
  ...
  GetTypeName: (GoodType) => {
    if (GoodType.includes("happiness")) {
      ...
      GoodType.includes("att_boost_attacker-all")
    ) {
      return i18n("Boxes.Productions.att_boost_attacker");
    } else if (
      GoodType.includes("att_boost_defender") ||
      GoodType.includes("att_boost_defender-all")
    ) {
      return i18n("Boxes.Productions.att_boost_defender");
    } else if (
      GoodType.includes("def_boost_attacker") ||
      GoodType.includes("def_boost_attacker-all")
    ) {
      return i18n("Boxes.Productions.def_boost_attacker");
    } else if (
      GoodType.includes("def_boost_defender") ||
      GoodType.includes("def_boost_defender-all")
    ) {
  ...
  efficiencyTT: (e) => {
    ...
    let y = Productions.ratedBuildings
      .filter((x) => !x.isInInventory && x?.rating?.[type] > 0)
      .map((x) => x.rating[type])
      .sort((a, b) => a - b);
  ...
  calculateFSP: (type, value) => {
    ...
    for (let x of Productions.FSPqualifiedResources) {
      if (
        Productions.Rating.Data.fsp[x] &&
        Productions.Rating.Data[x].active &&
        Productions.Rating.Data[x].perTile > 0
      ) {
        sum +=
          Productions.Rating.Data.fsp[x] / Productions.Rating.Data[x].perTile;
      }
    }
    if (type == "fsp") {
  ...
  CalcRatingBody: (era = "") => {
    ...
    Productions.BuildingsAll = Object.values(
    ...
    if (!Productions.AdditionalSpecialBuildings) {
      let spB = Object.values(MainParser.CityEntities).filter(
        (x) =>
          (x.is_special &&
            !["O_", "U_", "V_", "H_", "Y_"].includes(x.id.substring(0, 2))) ||
          x.id.substring(0, 11) === "W_MultiAge_",
      );
      Productions.AdditionalSpecialBuildings = {};
      for (x of spB) {
        Productions.AdditionalSpecialBuildings[x.id] = {
          id: x.id,
          name: x.name,
          selected: false,
          filter: x.id + ";" + x.name,
    ...
    let InventoryBuildings = (Productions.InventoryBuildings =
      Kits.BuildingsFromInventory());
    if (ActiveMap === "OtherPlayer") InventoryBuildings = [];
    ...
    for (const building of Productions.BuildingsAll) {
      if (
        ...
        building.id >= 2000000000 ||
        building.type.includes("hub")
      )
        continue;
      ...
      if (MainParser.Allies.buildingList?.[building.id] && withAllies)
        compare +=
          "+" +
          Object.keys(MainParser.Allies.buildingList?.[building.id]).join("+");

      let foundBuildingIndex = uniqueBuildings.findIndex(
        (x) =>
          x.name === compare &&
          x.isInInventory === building.isInInventory &&
          !MainParser.Allies.buildingList?.[x.id],
      );
      if (!withAllies)
        foundBuildingIndex = uniqueBuildings.findIndex(
          (x) =>
            x.name === compare && x.isInInventory === building.isInInventory,
        );
      ...
      if (foundBuildingIndex === -1) {
        ...
        if (
          buildingCount[building.entityId + inventoryIdentifier] === undefined
        )
        ...
        if (
          Technologies.InnoEras[foundBuilding.eraName] <
          Technologies.InnoEras[building.eraName]
        )
    ...
    let selectedAdditionals = Object.values(
      Productions.AdditionalSpecialBuildings,
    )
      .filter((x) => x.selected)
      .map((x) => x.id);

    Productions.ratedBuildings = ratedBuildings = Productions.rateBuildings(
      uniqueBuildings,
      false,
      era,
    ).concat(Productions.rateBuildings(selectedAdditionals, true, era));

    if (Productions.RatingCurrentTab === "Settings") {
      h.push(Productions.CalcRatingSettings());
    } else if (Productions.RatingCurrentTab === "Advice") {
      helper.preloader.show("#ProductionsRating");

      h.push('<div class="advice-section">');
      h.push('<div class="advice-controls dark-bg p5">');
      h.push(
        "<label>" + i18n("Boxes.ProductionsRating.ObjectiveLabel") + "</label>",
      );
      h.push(
        '<button class="btn' +
          (Productions.AdviceObjective === "forge_points" ? " active" : "") +
          '" data-objective="forge_points">' +
          i18n("Boxes.ProductionsRating.ObjectiveForgePoints") +
          "</button>",
      );
      h.push(
        '<button class="btn' +
          (Productions.AdviceObjective === "attack" ? " active" : "") +
          '" data-objective="attack">' +
          i18n("Boxes.ProductionsRating.ObjectiveAttack") +
          "</button>",
      );
      h.push(
        '<button class="btn' +
          (Productions.AdviceObjective === "defense" ? " active" : "") +
          '" data-objective="defense">' +
          i18n("Boxes.ProductionsRating.ObjectiveDefense") +
          "</button>",
      );
      h.push("</div>");
      h.push(Productions.GenerateAdviceHTML(Productions.AdviceObjective));
      h.push("</div>");
      ...
      for (const type of Productions.Rating.Types) {
        // skip inactive ones
        if (
          !Productions.Rating.Data[type]?.active ||
          Productions.Rating.Data[type]?.perTile === null
        )
          continue;
        ...
        if (type.includes("att_") || type.includes("def_")) {
          ...
          let twinType = type.includes("att_")
            ? "def_" + coreType
            : "att_" + coreType;

          if (combinedRatingTypes.find((x) => x.includes(combinedType)))
            continue;

          if (Productions.Rating.Data[twinType]?.active)
            combinedRatingTypes.push(combinedType);
          else combinedRatingTypes.push(type);
        }
      }

      let colNumber = Object.values(Productions.Rating.Data).filter(
        (x) => x.active && x.perTile != null,
      ).length;

      h.push('<div class="ratingtable">');
      h.push(
        '<a id="RatingsResults" class="toggle-tab btn btn-slim" data-value="Settings">' +
          i18n("Boxes.ProductionsRating.Settings") +
          "</a>",
      );
      h.push(
        '<a class="toggle-tab btn btn-slim" data-value="Advice">' +
          i18n("Boxes.ProductionsRating.Advice") +
          "</a>",
      );
      ...
      h.push(
        '<a class="btn" id="addMetaBuilding">' +
          i18n("Boxes.ProductionsRating.AddBuilding") +
          "</a>",
      );
      h.push(
        '<label for="tilevalues"><input type="checkbox" id="tilevalues" />' +
          i18n("Boxes.ProductionsRating.ShowValuesPerTile") +
          "</label>",
      );
      h.push(
        '<input type="text" id="efficiencyBuildingFilter" size=20 value="' +
          Productions.RatingSearchTerm +
          '" placeholder="' +
          i18n("Boxes.ProductionsRating.Filter") +
          ': neo|eden" />',
      );
      h.push(
        '<label for="showhighlighted" data-original-title="' +
          i18n("Boxes.ProductionsRating.ShowHighlightedExplanation") +
          '"><input type="checkbox" id="showhighlighted" />' +
          i18n("Boxes.ProductionsRating.ShowHighlighted") +
          "</label>",
      );
      h.push("<div>");
      h.push(
        '<label for="gBs" data-original-title="' +
          i18n("Boxes.ProductionsRating.NoGBsExplanation") +
          '"><input type="checkbox" id="gBs" /><img src="' +
          srcLinks.get(
            `/shared/gui/constructionmenu/icon_greatbuilding.png`,
            true,
          ) +
          '" /></label>',
      );
      if (ActiveMap !== "OtherPlayer") {
        h.push(
          '<div class="inventory">' +
            '<label for="inventorybuildings" data-original-title="' +
            i18n("Boxes.ProductionsRating.ShowInventoryBuildingsExplanation") +
            '"><input type="checkbox" id="inventorybuildings" /><img class="game-cursor" src="' +
            extUrl +
            'js/web/x_img/inventory.png"></label>' +
            '<label for="inventorybuildingscore" data-original-title="' +
            i18n("Boxes.ProductionsRating.InventoryBuildingScoreExplanation") +
            '">' +
            i18n("Boxes.ProductionsRating.InventoryBuildingScore") +
            ': <input type="number" size="6" value="' +
            Productions.efficiencySettings.inventorybuildingscore * 100 +
            '" id="inventorybuildingscore" /></label>' +
            '<label for="showLimited" data-original-title="' +
            i18n("Boxes.ProductionsRating.NoLimitedExplanation") +
            '"><input type="checkbox" id="showLimited" /><img src="' +
            srcLinks.get(
              `/shared/gui/upgrade/upgrade_icon_limited_building.png`,
              true,
            ) +
            '" /></label>' +
            "</div>",
        ...
        h.push(
          '<label for="showallies" data-original-title="' +
            i18n("Boxes.ProductionsRating.ShowAllies") +
            '"><input type="checkbox" id="showallies" ' +
            (Productions.efficiencySettings.showallies ? "checked" : "") +
            ' /><span class="filter showallies"></span></label>',
        );
      }
      h.push(
        '<label for="showitems" data-original-title="' +
          i18n("Boxes.ProductionsRating.ShowItems") +
          '"><input type="checkbox" id="showitems" /><span class="filter showitems"></span></label>',
      );
      h.push("</div></div></th>");
      h.push("</tr>");
      ...
      h.push(
        '<th data-type="ratinglist" class="is-number descending" data-export="' +
          i18n("Boxes.ProductionsRating.Score") +
          '">' +
          i18n("Boxes.ProductionsRating.Score") +
          "</th>",
      );
      h.push(
        '<th data-type="ratinglist" data-export="' +
          i18n("Boxes.ProductionsRating.BuildingName") +
          '"><div class="flex-between"><span>' +
          i18n("Boxes.ProductionsRating.BuildingName") +
          "</span>" +
          ' <div id="buildingsize"><span>' +
          i18n("Boxes.Productions.Headings.size") +
          "</span><ul>",
      );
      for (let size of buildingSizes) {
        h.push(
          '<li data-value="' +
            size +
            '" class="' +
            (Productions.RatingFilteredSizes.includes(size) ? "selected" : "") +
            '">' +
            size +
            "</li>",
        );
      ...
      h.push(
        '</ul></div></div></th><th data-type="ratinglist" class="is-number" data-export="#"></th><th class="no-sort inventory-buildings text-center"><img alt="" data-original-title="' +
          i18n("Boxes.ProductionsRating.InventoryTooltip") +
          '" class="game-cursor" src="' +
          extUrl +
          'js/web/x_img/inventory.png" /></th>',
      );
      ...
      for (const type of combinedRatingTypes) {
        ...
        h.push(
          '<th data-type="ratinglist" style="width:1%" data-export="' +
            Productions.GetTypeName(firstType) +
            '" class="is-number text-center buildingvalue"' +
            (secondType !== null
              ? ` data-original-title="${Productions.Rating.Data[firstType].perTile} + ${Productions.Rating.Data[secondType]?.perTile} / 2"`
              : "") +
            '><span class="resicon ' +
            firstType +
            '"' +
            (secondType === null ? ' style="margin-bottom:0"' : "") +
            "></span>" +
            (secondType === null ? "<br>" : "") +
            (secondType !== null
              ? '<span class="resicon ' + secondType + '"></span><i>'
              : "") +
            (Productions.Rating.Data[firstType].perTile +
              (Productions.Rating.Data[secondType]?.perTile || 0) || 0) /
              divider +
            "</i></th>",
        );
        ...
        h.push(
          '<th data-type="ratinglist" style="width:1%" data-export="' +
            Productions.GetTypeName(firstType) +
            '" class="is-number text-center tilevalue"' +
            (secondType !== null
              ? ` data-original-title="${Productions.Rating.Data[firstType].perTile} + ${Productions.Rating.Data[secondType]?.perTile} / 2"`
              : "") +
            '><span class="resicon ' +
      ...
      h.push(
        '<th data-type="ratinglist" data-export="Items" class="no-sort items">Items</th>',
      );
      h.push("</tr>");
      ...
      for (const building of ratedBuildings) {
        ...
        if (
          building.isInInventory &&
          building.rating.totalScore <
            Productions.efficiencySettings.inventorybuildingscore
        )
          continue;
        ...
        if (
          building.isInInventory &&
          (buildingCount[building.entityId + "C"] !== undefined ||
            buildingCount[building.entityId + "C"] >= 1)
        )
          continue;

        let buildingSize = building.size.length * building.size.width;

        [randomItems, randomUnits] = Productions.showBuildingItems(
          false,
          building,
        );
        h.push(
          `<tr class="${building.type === "greatbuilding" ? "gb " : ""}${building.isLimited ? "limited " : ""}${building.highlight ? "additional bg-blue " : ""}${building.isInInventory ? "inventory-building " : ""}size${buildingSize}">`,
        );
        h.push(
          '<td data-number="' +
            building.rating.totalScore * 100 +
            '" class="text-right">' +
            Math.round(building.rating.totalScore * 100) +
            "</td>",
        );
        ...
        if (!building.highlight && !building.isInInventory)
          h.push(
            '<span class="show-all" data-original-title="' +
              i18n("Boxes.General.ShowOnMap") +
              '" data-name="' +
              building.name +
              '"><img class="game-cursor" alt="" src="' +
              extUrl +
        ...
        h.push(
          '<span data-meta_id="' +
            ...
            building.rating.totalScore * 100 +
            '" data-era="' +
            (building.eraName === "AllAge" ? "" : building.eraName) +
            '" data-callback_tt="Tooltips.buildingTT" class="helperTT" ' +
            MainParser.Allies.tooltip(building.id) +
            ">" +
            building.name +
            "</span>",
        );

        let eraShortName = i18n(
          "Eras." + Technologies.Eras[building.eraName] + ".short",
        );
        if (eraShortName !== "-")
          h.push(
            " (" +
        ...
        let buildingAmount =
          MainParser.Allies.buildingList?.[building.id] && withAllies
            ? 1
            : buildingCount[building.entityId + "C"] || 1;
        h.push(
          '<td exportvalue="' +
        ...
        if (buildingAmount > 1)
          h.push(
            '<span data-original-title="' +
              i18n("Boxes.ProductionsRating.CountTooltip") +
              '">' +
              buildingCount[building.entityId + "C"] +
              "x</span>",
          );
        h.push("</div></td>");
        ...
        if (
          (buildingCount[building.entityId + "I"] !== undefined &&
            !building.isInInventory) ||
          building.isInInventory
        )
          h.push(
            '<span data-callback_tt="Kits.InventoryTooltip" data-id="' +
              building.entityId +
              '" class="helperTT"><img alt="" class="game-cursor" src="' +
              srcLinks.get(
                `/shared/gui/event_hub/event_meta_icon_checkmark.png`,
                true,
              ) +
              '" /></span> ',
          );
        ...
        for (const type of combinedRatingTypes) {
          ...
          if (secondType === null) {
            h.push(
              `<td class="text-right${firstType === "units" ? " units" : ""} buildingvalue" data-number="${Math.round(parseFloat(building.rating[firstType]))}" ${firstType === "units" ? `data-original-title="${randomUnits}"` : ""}>`,
            );
            h.push(HTML.Format(building.rating[firstType]));
            h.push("</td>");

            let roundingFactor =
              building.rating[firstType + "-tile"] > 100 ||
              building.rating[firstType + "-tile"] < -100
                ? 1
                : 100;
            let tileValue =
              Math.round(
                building.rating[firstType + "-tile"] * roundingFactor,
              ) / roundingFactor;
            h.push(
              `<td class="text-right${firstType === "units" ? " units" : ""} tilevalue" data-number="${tileValue}" ${firstType === "units" ? `data-original-title="${randomUnits}"` : ""}>`,
            );
          ...
          // combined attack boosts
            ...
            h.push(
              `<td class="text-right buildingvalue" data-number="${combinedValue}">`,
            );
            h.push(
              HTML.Format(
                building.rating[firstType] + building.rating[secondType],
              ),
            );
            h.push("</td>");
            ...
            let roundFirstTile =
              firstTileValue > 100 || firstTileValue < -100
                ? Math.round(firstTileValue)
                : Math.round(firstTileValue * 100) / 100;

            let roundSecondTile =
              secondTileValue > 100 || secondTileValue < -100
                ? Math.round(secondTileValue)
                : Math.round(secondTileValue * 100) / 100;

            let tileValue =
              Math.round((roundFirstTile + roundSecondTile) * 10) / 10;

            h.push(
              `<td class="text-right tilevalue" data-number="${tileValue}">`,
            );
            h.push(HTML.Format(tileValue));
            h.push("</td>");
          }
      ...
      h.push("</tbody>");
      h.push(
        '<tfoot><tr class="highlighted-explained"><td colspan="' +
          (colNumber + 3) +
          '">' +
          i18n("Boxes.ProductionsRating.HighlightsExplained") +
          "</td></tr></tfoot>",
      );
      h.push("</table>");
      h.push(
        '<div class="overlay"><a class="window-close closeMetaBuilding"></a>',
      );
      h.push('<div class="content">');
      h.push(
        '<input id="findMetaBuilding" placeholder="' +
          i18n("Boxes.ProductionsRating.FindSpecialBuilding") +
          '" value="">',
      );
      h.push('<ul class="results"></ul>');
      h.push('<div class="btns">');
      h.push(
        '<a class="btn selectMetaBuildings">' +
          i18n("Boxes.ProductionsRating.ToggleBuildingSelection") +
          "</a>",
      );
      h.push(
        '<a class="btn closeMetaBuilding btn-green">' +
          i18n("Boxes.ProductionsRating.AddBuildings") +
          "</a>",
      );
      h.push("</div>");
      h.push("</div>");
    ...
    SaveSettings = (x) => {
      ...
      localStorage.setItem(
        "Productions.efficiencySettings",
        JSON.stringify(Productions.efficiencySettings),
      );
    ...
    $("#ProductionsRatingBody")
      ...
      .done(function () {
        ...
        $(".ratingPresetDelete").on("click", () => {
          ...
          if (!preset) return;
          if (
            !window.confirm(i18n("Boxes.ProductionsRating.PresetConfirmDelete"))
          )
            return;
          const activeId = Productions.Rating.Presets?.activePresetId;
        ...
        $("#ratingPresetReset").on("click", () => {
          if (
            !window.confirm(i18n("Boxes.ProductionsRating.PresetConfirmReset"))
          )
            return;
        ...
        $("#inventorybuildings, label[inventorybuildings]").on(
          "click",
          function () {
        ...
        if (
          Productions.efficiencySettings.tilevalues !==
          $("#tilevalues").is(":checked")
        )
          $("#tilevalues").trigger("click");
        if (
          Productions.efficiencySettings.showitems !==
          $("#showitems").is(":checked")
        )
          $("#showitems").trigger("click");
        if (
          Productions.efficiencySettings.showhighlighted !==
          $("#showhighlighted").is(":checked")
        )
          $("#showhighlighted").trigger("click");
        if (
          Productions.efficiencySettings.inventorybuildings !==
          $("#inventorybuildings").is(":checked")
        )
          $("#inventorybuildings").trigger("click");
        if (Productions.efficiencySettings.gBs !== $("#gBs").is(":checked"))
          $("#gBs").trigger("click");
        if (
          Productions.efficiencySettings.showLimited !==
          $("#showLimited").is(":checked")
        )
          $("#showLimited").trigger("click");
        ...
        let filterMeta = (regEx) => {
          ...
          let foundBuildings = Object.values(
            Productions.AdditionalSpecialBuildings,
          )
            .filter((x) => regEx.test(x.filter))
            .sort(
              (a, b) =>
                (a.selected !== b.selected ? (a.selected ? -2 : 2) : 0) +
                (a.name > b.name ? 1 : -1),
            );

          for (building of foundBuildings) {
            $("#ProductionsRatingBody .overlay .results").append(
              `<li data-meta_id="${building.id}" data-era="${era === "AllAge" ? "" : era}" data-callback_tt="Tooltips.buildingTT" class="helperTT${building.selected ? " selected" : ""}">${building.name}</li>`,
            );
          }
        };
        filterMeta(/./);
        $("#ProductionsRatingBody .overlay .results").on("click", "li", (e) => {
          ...
          Productions.AdditionalSpecialBuildings[id].selected =
            !Productions.AdditionalSpecialBuildings[id].selected;
          e.target.classList.toggle("selected");
        });
        $("#ProductionsRatingBody .overlay .selectMetaBuildings").on(
          "click",
          (e) => {
        ...
        $("#ProductionsRatingSettings input[type=checkbox]").on(
          "click",
          function () {
            let elem = $(this);
        ...
        $("#ProductionsRatingSettings input[type=number]").on(
          "blur",
          function () {
            let elem = $(this);
            ...
            Productions.calculateFSP(
              type,
              Productions.Rating.Data[type].perTile,
            );
            Productions.Rating.save();
          },
        );
          ...
          $(h)
            ...
            .done(() => {
              $("#FSPCalculator input").on("input", (e) => {
                ...
                Productions.Rating.Data.fsp[type] =
        ...
        $("#buildingsize li").on("click", (e) => {
          ...
          if (
            ...
            Productions.RatingFilteredSizes.length === 0
          ) {
            $(".ratinglist tr").removeClass("hidden");
            return;
          }
          $(".ratinglist tr").each((i, elem) => {
            let size = Array.from(elem.classList).find((x) =>
              x.includes("size"),
            );
            if (size) {
        ...
        $("#ProductionsRatingBody [data-original-title]").tooltip({
          container: "#game_body",
          html: true,
        });
        ...
        // Advice: objective buttons
        $(".advice-controls button[data-objective]").on("click", function () {
          Productions.AdviceObjective = $(this).data("objective");
          $(".advice-controls button").removeClass("active");
          $(this).addClass("active");
          $(".advice-section").html(
            Productions.GenerateAdviceHTML(Productions.AdviceObjective),
          );
        });
  ...
  CalcRatingSettings: () => {
    ...
    h.push(
      '<a id="RatingsResults" class="toggle-tab btn btn-slim" data-value="Results"><span>' +
        i18n("Boxes.ProductionsRating.Results") +
        "</span></a>",
    );
    h.push(
      '<a class="toggle-tab btn btn-slim" data-value="Advice"><span>' +
        i18n("Boxes.ProductionsRating.Advice") +
        "</span></a>",
    );
    Productions.Rating.ensurePresets();
    ...
    h.push("</div>");
    h.push(
      '<input type="file" id="ratingPresetImportFile" accept="application/json" style="display:none" />',
    );
    ...
    h.push(
      '<span class="text-right">' +
        i18n("Boxes.ProductionsRating.ProdPerTile") +
        "</span>",
    );
    h.push("</li>");
    ...
    for (let type of Productions.Rating.Types) {
      if (type === "guild_raids_action_points_collection")
        h.push(
          `<li class="heading">${i18n("Boxes.General.Quantum_Incursion")}</li>`,
        );
      else if (type === "units")
      ...
      let activeSetting =
        Productions.Rating.Data[type]?.perTile !== null &&
        Productions.Rating.Data[type]?.active !== false;
      h.push(
        '<input id="Enabled-' +
          type +
          '" class="no-grow enabled game-cursor" ' +
          (activeSetting ? "checked" : "") +
          ' type="checkbox">',
      );
      ...
      h.push(
        '<label for="Enabled-' +
          type +
          '">' +
          Productions.GetTypeName(type) +
          "</label>",
      );
      if (type == "fsp")
        h.push(
          `<span id="ShowFSPCalculator" class="clickable" data-original-title="${i18n("Boxes.ProductionsRating.ShowFSPCalculator")}">🧮</span>`,
        );
      h.push(
        '<input type="number" id="ProdPerTile-' +
          type +
          '" step="0.01" min="0" max="1000000" class="no-grow helperTT ' +
          (Productions.Rating.Data[type]?.active ? "" : "hidden") +
          '" value="' +
          (Productions.Rating.Data[type]?.perTile || 0) +
          '", data-callback_tt="Productions.efficiencyTT", data-type="' +
          type +
          '-tile">',
      );
      h.push("</li>");
    ...
    h.push('<div class="content">');
    h.push(
      '<a class="toggle-tab btn btn-green" data-value="Results">' +
        i18n("Boxes.ProductionsRating.Results") +
        "</a>",
    );
    ...
    h.push(
      `<button class="btn" id="ratingPresetExport">${i18n("Boxes.ProductionsRating.PresetExport")}</button>`,
    );
    h.push(
      `<button class="btn" id="ratingPresetImport">${i18n("Boxes.ProductionsRating.PresetImport")}</button>`,
    );
    h.push("</div>");
    h.push("</div>");
    h.push("</div>");
  ...
  rateBuildings: (uniqueBuildings, additional = false, era = null) => {
    let ratedBuildings = [];
    if (additional) {
      uniqueBuildings = uniqueBuildings.map((x) =>
        CityBuildings.createBuilding(x, era || CurrentEra),
      );
    }
    for (const building of uniqueBuildings) {
      // do not include wishingwell type buildings
      if (
        building.entityId.includes("L_AllAge_EasterBonus1") ||
        building.entityId.includes("L_AllAge_Expedition16") ||
        building.entityId.includes("L_AllAge_ShahBonus17") ||
        (building.isSpecial === undefined && building.type !== "greatbuilding")
      )
        continue;
  ...
  rateBuilding: (building) => {
    ...
    size +=
      building.size.width === 1 || building.size.length === 1
        ? building.needsStreet * 0.5
        : building.needsStreet;

    let score = { totalScore: 0 };

    for (const type of Object.keys(Productions.Rating.Data)) {
      if (Productions.Rating.Data[type].active) {
        let desiredValuePerTile =
          parseFloat(Productions.Rating.Data[type].perTile) || 0;
        if (desiredValuePerTile !== null && !isNaN(desiredValuePerTile)) {
          let typeValue =
            Productions.getRatingValueForType(building, type) || 0; // production amount
  ...
  getRatingValueForType: (building, type) => {
    if (type === "happiness") return building.happiness;
    else if (type === "population") return building.population;
      ...
      for (const boost of building.boosts) {
        ...
        if (boost.needsLink && building.setBuilding !== undefined) {
          if (
            boost.requiredLinks >
            (building.setBuilding.uniqueAdjacentCount || 0)
          )
            continue;
        }
      ...
      for (const boost of building.boosts) {
        ...
        if (
          boost.type[0] === "forge_points_production" &&
          type === "forge_points_production"
        ) {
          return boost.value;
        }
        if (
          boost.type[0] === "goods_production" &&
          type === "goods_production"
        ) {
          return boost.value;
        }
      ...
      return Productions.getBuildingProductionByCategory(false, building, type)
        .amount;
    else if (type.includes("goods") && !type.includes("guild_raids_")) {
      let allGoods = CityBuildings.getBuildingGoodsByEra(false, building);
      let era =
        ActiveMap === "OtherPlayer" ? CityMap.OtherPlayer.eraName : CurrentEra;
      ...
      if (allGoods !== undefined) {
        ...
        if (type === "goods-previous") {
          if (allGoods.eras[prevEra]) return allGoods.eras[prevEra];
        }
        if (type === "goods-current") {
          if (allGoods.eras[currEra]) return allGoods.eras[currEra];
        }
        if (type === "goods-next") {
          if (allGoods.eras[nextEra]) return allGoods.eras[nextEra];
        }
      }
    } else if (type === "fsp") {
      let fsp = 0;
      if (building.production) {
        let possibleProductions = building.production
          .filter((x) => x.type === "genericReward")
          .concat(building.production.filter((x) => x.type === "random"));
        let multiplier = 1;
        for (let production of possibleProductions) {
          if (production.type === "genericReward") {
            if (
              !production.resources.id.includes("rush_event_buildings_instant")
            )
              continue;
            ...
            let hasFsp = production.resources.filter((x) =>
              x.id?.includes("rush_event_buildings_instant"),
            );
  ...
  ShowSettings: () => {
    ...
    let show24Time =
      showAMPMTime === false && showRelativeProductionTime === false;

    let h = [];
    h.push(
      `<p><input id="productionsShowRelativeTime" name="productionTime" value="1" type="radio" ${showRelativeProductionTime === true ? ' checked="checked"' : ""} /> <label for="productionsShowRelativeTime">${i18n("Boxes.Productions.RelativeTime")}</label><br>`,
    );
    h.push(
      `<input id="productionsShowAMPMTime" name="productionTime" value="1" type="radio" ${showAMPMTime === true ? ' checked="checked"' : ""} /> <label for="productionsShowAMPMTime">${i18n("Boxes.Productions.AMPMTime")}</label><br>`,
    );
    h.push(
      `<input id="productionsShow24Time" name="productionTime" value="1" type="radio" ${show24Time === true ? ' checked="checked"' : ""} /> <label for="productionsShow24Time">${i18n("Boxes.Productions.Time24")}</label></p>`,
    );
    h.push(
      `<p><button onclick="Productions.SaveSettings()" id="save-productions-settings" class="btn" style="width:100%">${i18n("Boxes.Settings.Save")}</button></p>`,
    );

    let activeTable = $("#ProductionsBody .horizontal li.active")
      .attr("id")
      .replace("prod-", "");
  ...
  RSettings: () => {
    let c = [];
    c.push(
      `<p class="text-left">${i18n("Boxes.General.Export")}: <span class="btn-group"><button class="btn" onclick="HTML.ExportTable($('.ratingtable table'),'csv','EfficiencyRating')">CSV</button>`,
    );
    c.push(
      `<button class="btn" onclick="HTML.ExportTable($('.ratingtable table'),'json','EfficiencyRating')">JSON</button></span></p>`,
    );
  ...
  SaveSettings: () => {
    ...
    if ($("#productionsShowRelativeTime").is(":checked"))
      showRelativeProductionTime = true;
    localStorage.setItem(
      "productionsShowRelativeTime",
      showRelativeProductionTime,
    );
  ...
  buildingItemList: () => {
    let temp = Object.assign(
      {},
      ...Object.values(MainParser.CityEntities)
        .filter((b) => b.id[0] === "W")
        .map((x) => ({
          [x.id]: [
            ...JSON.stringify(x).matchAll(
              /"id":"([^"]*?)"[^()[\]{}]*?"name":"([^"]*?)"[^()[\]{}]*?"iconAssetName":"([^"]*?)"[^{}]*?"__class__":"(GenericReward|TimedReward)"/gm,
            ),
          ].map((a) => ({ id: a[1], name: a[2], icon: a[3] })),
        })),
    );
    ...
    for (let [building, list] of Object.entries(temp)) {
      for (let item of list) {
        ...
        if (items[item.id]) {
          if (!items[item.id].buildings.includes(building))
            items[item.id].buildings.push(building);
        } else {
          items[item.id] = {
            name: item.name,
            buildings: [building],
  ...
  getBuildingsByBoosts: (boostArray = []) => {
    let buildings = Object.values(MainParser.CityEntities).filter(
      (b) => b.id[0] === "W",
    );
    let boostList = {};
    ...
    for (let building of buildings) {
      ...
      if (buildingAABoost === undefined && buildingCABoost === undefined)
        continue;
      ...
      for (let boost of boostArray) {
        ...
        if (foundAllABoost === undefined && foundCurrentABoost === undefined)
          continue;

        if (boost.includes("guild_raids") && building.id.includes("GuildRaids"))
          continue;
  ...
  createBuildingBoostList: (boostArray = []) => {
    ...
    for (let [group, buildings] of Object.entries(groupedBuildings)) {
      h +=
        '<tr><td><h2><span class="boost ' +
        group +
        '"></span> ' +
        i18n("Boxes.BoostList." + group) +
        "</h2><ul>";
      for (let building of buildings) {
        h +=
          '<li class="helperTT" data-era="' +
          CurrentEra +
          '" data-callback_tt="Tooltips.buildingTT" data-meta_id="' +
          building.entityId +
          '">' +
          building.name +
          "</li>";
  ...
  /**
   * Generates building recommendations based on the selected objective (FP, Attack, or Defense)
   *
   * @param {string} objective - The building objective: 'forge_points', 'attack', or 'defense'
   * @returns {Object} Recommendations object with top 10 buildings and analysis
   */
  BuildingAdvice: (objective = "forge_points") => {
    if (
      !Productions.ratedBuildings ||
      Productions.ratedBuildings.length === 0
    ) {
      return {
        error: "No rated buildings available",
        recommendations: [],
      };
    }

    // Map objectives to rating types
    const objectiveMap = {
      forge_points: ["strategy_points", "forge_points_production"],
      attack: [
        "att_boost_attacker-all",
        "att_boost_attacker-battleground",
        "att_boost_attacker-guild_expedition",
    ...
    const relevantTypes =
      objectiveMap[objective] || objectiveMap["forge_points"];

    // Get city building counts for comparison
    let buildingCount = {};
    for (let building of Object.values(MainParser.CityMapData)) {
      let entityId = building.cityentity_id;
      if (!buildingCount[entityId]) {
        buildingCount[entityId] = 0;
      }
      buildingCount[entityId]++;
    }

    // Calculate advice score for each building based on objective
    let buildingAdvices = Productions.ratedBuildings
      .filter((b) => !b.isInInventory) // Filter out inventory-only buildings for now
      .map((building) => {
        ...
        for (let type of relevantTypes) {
          let score = building.rating[type] || 0;
          objectiveBreakdown[type] = score;
          objectiveScore += score;
        }

        // Calculate building size efficiency
        let size = building.size.width * building.size.length;
        let scorePerTile = objectiveScore / size;

        // Count how many already in city
        let countInCity = buildingCount[building.entityId] || 0;
        ...
        return {
          ...
          currentCount: countInCity,
          isInInventory: building.isInInventory,
        };
      })
      // Sort by objective score (descending), then by score per tile (descending)
      .sort((a, b) => {
        if (b.objectiveScore !== a.objectiveScore) {
          return b.objectiveScore - a.objectiveScore;
        }
        return b.scorePerTile - a.scorePerTile;
      });

    // Get inventory buildings separately
    let inventoryBuildings = Productions.ratedBuildings
      .filter((b) => b.isInInventory)
      .map((building) => {
        let objectiveScore = 0;
        let objectiveBreakdown = {};
    ...
    // Combine: inventory first (high priority), then city buildings
    let allRecommendations = [...inventoryBuildings, ...buildingAdvices];

    // Get top 10 recommendations
    let topRecommendations = allRecommendations.slice(0, 10);

    // Calculate statistics
    let avgScore = 0;
    if (topRecommendations.length > 0) {
      avgScore =
        topRecommendations.reduce((sum, b) => sum + b.objectiveScore, 0) /
    ...
    return {
      ...
      totalEvaluated: Productions.ratedBuildings.length,
      averageObjectiveScore: Math.round(avgScore * 100) / 100,
      buildingStats: {
        inInventory: inventoryBuildings.length,
        inCity: buildingAdvices.length,
      },
  ...
  /**
   * Generates HTML content for the building advice section
   *
   * @param {string} objective - The building objective: 'forge_points', 'attack', or 'defense'
   * @returns {string} HTML string for the advice display
   */
  GenerateAdviceHTML: (objective = "forge_points") => {
    let advice = Productions.BuildingAdvice(objective);

    if (advice.error) {
      return `<div class="advice-error">${advice.error}</div>`;
    }

    let html = [];
    let objectiveLabel = {
      forge_points:
        i18n("Boxes.ProductionsRating.ObjectiveFP") || "Forge Points",
      attack: i18n("Boxes.ProductionsRating.ObjectiveAttack") || "Attack Power",
      defense:
        i18n("Boxes.ProductionsRating.ObjectiveDefense") || "Defense Power",
    };

    html.push('<div class="building-advice">');
    html.push(`<h3>${objectiveLabel[objective] || objective}</h3>`);
    html.push(
      `<div class="advice-stats">Average Score: ${advice.averageObjectiveScore}</div>`,
    );
    html.push('<table class="foe-table advice-table">');
    html.push("<thead>");
    html.push("<tr>");
    html.push("<th>Priority</th>");
    html.push("<th>Building</th>");
    html.push("<th>Location</th>");
    html.push("<th>Score</th>");
    html.push("<th>Per Tile</th>");
    html.push("<th>In City</th>");
    html.push("</tr>");
    html.push("</thead>");
    html.push("<tbody>");

    for (let i = 0; i < advice.topRecommendations.length; i++) {
      let building = advice.topRecommendations[i];
      let priority = building.isInInventory ? "🎁 Inventory" : i + 1 + ".";

      html.push(
        '<tr class="' +
          (building.isInInventory ? "inventory-priority" : "") +
          '">',
      );
      html.push(`<td class="text-center">${priority}</td>`);
      html.push(`<td>${building.name}</td>`);
      html.push(`<td>${building.eraName || "AllAge"}</td>`);
      html.push(
        `<td class="text-right">${Math.round(building.objectiveScore * 100) / 100}</td>`,
      );
      html.push(
        `<td class="text-right">${Math.round(building.scorePerTile * 100) / 100}</td>`,
      );
      html.push(
        `<td class="text-center">${building.currentCount > 0 ? building.currentCount + "x" : "-"}</td>`,
      );
      html.push("</tr>");
    }
